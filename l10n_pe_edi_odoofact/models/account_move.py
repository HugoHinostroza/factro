...
class AccountMove(models.Model): 
    ...
    l10n_pe_edi_picking_number = fields.Char(string='Picking number')
    l10n_pe_edi_reversal_serie = fields.Char(string='Document serie', compute='_get_reversed_entry', help='Used for Credit and debit note', readonly=False, store=True)
    l10n_pe_edi_reversal_number = fields.Char(string='Document number', compute='_get_reversed_entry', help='Used for Credit and debit note', readonly=False, store=True)
    l10n_pe_edi_reversal_date = fields.Char(string='Document date', compute='_get_reversed_entry', help='Date of the Credit or debit note', readonly=False, store=True)
    ...
    l10n_pe_edi_amount_igv = fields.Monetary(string='IGV Amount', store=True, compute='_compute_edi_amount', track_visibility='always')
    l10n_pe_edi_amount_others = fields.Monetary(string='Other charges', store=True, compute='_compute_edi_amount', track_visibility='always')  
    ...
    def action_post(self):
        if self.l10n_pe_edi_is_einvoice and self.amount_total > 700 and not self.partner_id.vat:
    ...
    def _get_invoice_values_odoofact(self):
        ...
        values = {
            ...
            'cliente_tipo_de_documento': self.partner_id.l10n_latam_identification_type_id and self.partner_id.l10n_latam_identification_type_id.l10n_pe_vat_code or '1',
            'cliente_numero_de_documento': self.partner_id.vat and self.partner_id.vat or '00000000',
            'cliente_denominacion': self.partner_id.name or self.partner_id.name,
